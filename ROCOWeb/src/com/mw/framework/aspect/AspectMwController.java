...
@Aspect
@Component
public class AspectMwController {
	private static final Logger logger = LoggerFactory.getLogger(AspectMwController.class);

	@Pointcut("execution(* com.mw.framework.controller..*.*(..))")
	private void anyMethod1() {
	}

	@Around("anyMethod1() && @annotation(org.springframework.web.bind.annotation.RequestMapping) && !@annotation(com.mw.framework.json.filter.annotation.IgnoreProperties)")
	public Object doAround(ProceedingJoinPoint pjp) throws Throwable {
		...
		//System.out.println("-=-=-=-=Method["+pjp.getTarget().getClass()+"."+pjp.getSignature().getName()+"]["+sdf.format(new Date())+"]["+request.getRequestURI()+"]["+currUser+"]["+na+"]["+ia+"]=-=-=-==-=-=-=-=--=-=-");

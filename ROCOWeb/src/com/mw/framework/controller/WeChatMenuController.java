 ...
 */
@Controller
@RequestMapping("/core/wechatmenu/*")
public class WeChatMenuController extends GenericController<WeChatMenu>{
	...
	@IgnoreProperties(value={@IgnoreProperty(name = { "hibernateLazyInitializer", "handler", "fieldHandler","children"}, pojo = WeChatMenu.class),
			@IgnoreProperty(name = { "hibernateLazyInitializer", "handler", "fieldHandler", "parent","children","users","menus"}, pojo = WeChat.class)})
	@ResponseBody
	public Message get(@PathVariable String id){
		Message msg = null;
		try {
			WeChatMenu menu = commonManager.getById(id, WeChatMenu.class);
	...
	public Message getForRoles(@PathVariable String id){
		Message msg = null;
		WeChatMenu menu = commonManager.getOne(id, WeChatMenu.class);

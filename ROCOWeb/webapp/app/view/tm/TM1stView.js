Ext.define('SMSWeb.view.tm.TM1stView', {
	...
	initComponent:function(){
		...
		//获取编辑表单数据
		Ext.Ajax.request({
			url:'/core/ext/grid/form/'+xmlName,
			async:false,
			dataType: "json",
		...
		if(configFields.xtype=="uxgrid"){
			...
			uxgrid =  Ext.widget('uxgrid',{
	        	...
	        	pageToolbar:true,
	        	selModelStatus:configFields.selModelStatus=="false"?null:"true",
//	        	gridBuffered:true,// 加上会有EXTjs异常 不启用
	        	gridGroupField:configFields.gridGroupField,
	        	gridSum:configFields.gridSum,
	        	region : 'center',
				listeners:{
				itemdblclick:function(ths , record , item ,index , e , eOpts){
						if(ths.id=='gridview-1052'){
							var update=Ext.create('SMSWeb.view.tm.TMUpdateSave',{
								dataRecord:record.data.id
							});
							update.show();
							
						}
						if(ths.id=='gridview-1065'){
							var update=Ext.create('SMSWeb.view.tm.TM1UpdateSave',{
								dataRecord:record.data.id,orderCode:record.data.orderCode,saleId:record.data.saleId
							});
/*							var update=Ext.create('SMSWeb.view.sale.NewSaleContentComplainidWindow',{
								loadStatus:'2',title:'客诉报表',sourceShow:"TM1stView",shId:record.data.id,source:'update'
							});*/
							update.show();
							
						}
						
						if(ths.id=='gridview-1025'){
							var update=Ext.create('SMSWeb.view.tm.CustomerServiceCapacityView',{
								assignee:record.data.assignee,ytime:record.raw.ytime
							});
							update.show();
							
						}
						
						if(ths.id=='gridview-1056'){
							var orderCode=searchForm.getForm().findField("orderCode").getValue();
							Ext.create('SMSWeb.view.tm.TM2OrdErrType',{
								title:'修改',orderCodePosex:record.data.orderCodePosex,acoeId:record.data.acoeId
								,errTypeDesc:record.data.errTypeDesc
								,tackit:record.data.tackit,errDesc:record.data.errDesc
							}).show();
						}
							//店面已下单未报价明细表
						if(ths.id=='gridview-1029'){
							var end_=$("input[name=orderDate]:eq(1)").val();
							var start_=$("input[name=orderDate]:eq(0)").val();
							var kun_=searchForm.getForm().findField("kunnr").getValue();
							Ext.create('SMSWeb.view.tm.OrderReportView',{
								startTime:start_,
								endTime:end_,
								kunnr:record.data.kunnr
							}).show();
						}
					...
					fileDownloadButtonClick:function(grid,rowIndex,colIndex){
						var record=grid.getStore().getAt(rowIndex);
						me.fileDownload(me,record.getId(),configFields);
					},
			...
			//uxgrid.getStore().group('money');
			//第一次的时候查询数据
			Ext.apply(uxgrid.getStore().proxy.extraParams,searchForm.getSearchs());
			//autoLoad==true/false 是否加载数据 2016-05-18
			if("true"==configFields.autoLoad){
		...
		toolbar.add('-');
		//手动设置工具栏按钮
		var curd = configFields.curd;
		
		if((curd && curd.indexOf("D")!=-1)||curd==undefined){
			toolbar.add({
				xtype : 'buttontransparent',
				text : '删除',
				glyph : 0xf00d,
		...
		if((curd && curd.indexOf("C")!=-1)||curd==undefined){
			toolbar.add({
				xtype : 'buttontransparent',
				text : '新增',
				glyph : 0xf067,
				//iconCls : 'table_add',
				handler:function(){
					if(configFields.formView){
						me.createWindowCustomForm(me,uxgrid,searchForm,configFields);
					}else{
						me.createWindowForm(me,formFields,null,uxgrid,searchForm,configFields);
					}
					
				}
			});
		}
		if(xmlName.indexOf("SYS_USER")!=-1){
			toolbar.add({
				xtype : 'buttontransparent',
				text : '批量授权分区',
				glyph : 0xf063,
				...
				handler:function(){
					...
					if(ids.length==0){
						Ext.Msg.alert("提示","请选中需授权的用户!");
						return;
					}
					var update=Ext.create('SMSWeb.view.tm.RearFrom',{
						dataRecord:ids,
						title:'批量授权分区',
						listeners:{
							click:function(){
		...
		//Excel导出
		if((curd && curd.indexOf("E")!=-1)||curd==undefined){
			toolbar.add({
				xtype : 'buttontransparent',
				text : '导出',
//				glyph : 0xf067,
				...
				handler:function(){
					Ext.MessageBox.confirm("温馨提示", "导出到Excel", function (btn) {
						if(btn=="yes"){
//							 alert("excel");
//							window.location.href = basePath+'/core/report/exportExcel/'+xmlName; 
							console.info(searchForm.getSearchs());
							var query=searchForm.getSearchs();
							var content='excelName='+configFields.title;  
							for(var key in query){  
								content +='&'+key+'='+query[key];
							} 
//							console.info(content);
							var url=basePath+'core/report/exportExcel/'+xmlName+"?"+content;
							window.open(url,configFields.title+"导出Excel",'fullscreen');
							//ExportExcelByGrid(uxgrid);
						}
					});
				}
			});
...
//				glyph : 0xf067,
				...
				listeners:{
					click:function(){
						Ext.create('SMSWeb.view.tm.CustSoftDogUploadWindow',{formId : id,title:"文件上传"}).show();
					}
				}
			});
		}
		if(CURR_USER_LOGIN_NO=="admin"){
			toolbar.add({
				...
				text : '更新模板',
				handler:function(){
					window.location="/core/tm/1st/"+xmlName+"?xxx=up";
				}
			});
		}
		if(xmlName=="RP_STAYORD_PAY"){
			toolbar.add({
				xtype:'buttontransparent',
				glyph:0xf085,
				text : '处理',
				listeners:{
					click:function(){
						...
						var is_stay=[];
						for(var i = 0; i < selectedRows.length ; i++){
							ids[i]=selectedRows[i].data.orderCode;
							var a=Number(selectedRows[i].data.day);
							if(Number(selectedRows[i].data.day)<7){
								Ext.Msg.alert("提示","选中订单中遗留天数包含小于7天!");
								return;
							}
						}
						if(ids.length==0){
							Ext.Msg.alert("提示","请选中需要处理的数据!");
							return;
						}
						Ext.Ajax.request({
							url:'main/saleReport/stayOrderPay',
							async:false,
							params:{ids:ids},
							method:'POST',
							dataType: "json",
	...
	createWindowCustomForm:function(me,grid,searchForm,config){
		...
		Ext.create('Ext.window.Window',{
            ...
            plain : true,
            border:false,
			items:[Ext.create(config.formView)],
			buttons: [{
			            text: '重置表单',
			            itemId:'resetForm'
			        }, {
			...
			listeners:{
				close:function(){
					if(config.xtype=="uxgrid"){
//						grid.getStore().loadPage(1,{params:searchForm.getSearchs()});
						grid.getStore().reload({params:searchForm.getSearchs()});
					}else{
			...
			dataform.load({
				...
				success:function(f,action){
					...
					Ext.Object.each(result.data,function(key, value, myself){
						...
						dataform.getForm().findField("telNumber").setValue(myself.telf1);
					});
					//alert(0);
	...
	createGantModuleuxGrid:function(configFields,item,record){
		var uxgridModule =  Ext.widget('uxgrid',{
        	...
        	gridBuffered:true,
        	viewConfig:{
			    enableTextSelection:false //可以复制单元格文字
			},
        	gridLoad:true,
        	title:item.title,
	...
	fileDownload:function(meheader,recordId,configFields){
		window.location.href=basePath+configFields.fileDownloadPath+recordId;
	},

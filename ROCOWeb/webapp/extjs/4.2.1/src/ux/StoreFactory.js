...
Ext.define('Ext.ux.StoreFactory',{
	...
	getStore : function(map){
		...
		var rootName = map.rootName == undefined || map.rootName == null ? ux.rootName : map.rootName;
		...
		var params = map.params == undefined || map.params == null ? {} : map.params;
 		...
 		var newStore = null;
 		if(modelType == 1){
			newStore = Ext.define(ux.createStore(storeName),{//声明一个Ext.data.Store对象
			...
			newStore = Ext.define(ux.createStore(storeName),{//声明一个Ext.data.TreeStore对象
...
//实例化一个storeFactory对象
Ext.ux.storeFactory = Ext.create('Ext.ux.StoreFactory');

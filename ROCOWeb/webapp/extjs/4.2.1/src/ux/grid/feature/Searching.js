...
Ext.define('Ext.ux.grid.feature.Searching', {  
    ...
    ,onKeyUp:function() {  
        ...
        if(0 === length || this.minChars <= length) {  
            //this.onTriggerSearch();  //禁用掉该事件，以免无谓的多次触发事件
        }  
    } // eo function onKeyUp  
    // }}}  
    // {{{  
    ...
    ,onTriggerSearch:function() {  
        ...
        // your proxy must be a Server proxy  
            ...
            /*if(store.lastOptions && store.lastOptions.params) {  
                store.lastOptions.params[store.lastOptions.params.start] = 0;  
            }  */
            ...
            if(fields.length) {  
                ...
                proxy.extraParams[this.paramNames.query] = (encodeURIComponent(val));  
            }  
            ...
            // reload store  
            store.loadPage(1); //不能直接load数据，需要把分页给回第一页
        }  

<#assign types={"width":"int", "filterable":"boolean", "selectOnFocus":"boolean","hidden":"boolean","editable":"boolean"}/>
<#compress>
[
<#list doc.grid.head.column as column>
	...
	{
		<#list column.@@ as attr>
			<#if types[attr?node_name]?exists>${attr?node_name}:${attr}<#else>${attr?node_name}:'${attr}'</#if><#if attr_has_next>,</#if>
		</#list>
				...
				<#list item.@@ as attr>
					<#if attr?node_name=='handler'>
						${attr?node_name}:function(grid,rowIndex,colIndex){
							this.up('treepanel').fireEvent('${attr}',grid,rowIndex,colIndex);
						}
					<#else>
